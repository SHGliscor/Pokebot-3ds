...
class Bridge:
    ...
    def input_ping(self) -> dict:
        ...
        return {
            ...
            "hid_latch": bool(caps & INPUT_CAP_HID_LATCH),
        }
    ...
    def hid_latch_no_retransmit(self, raw_hid: int) -> dict:
        """
        Send command 10 exactly once and prove that firmware has latched the
        requested HID state. The latch remains IN_PROGRESS until RELEASE_ALL.

        A lost UDP acknowledgement is ambiguous, so command 10 is NEVER
        blindly retransmitted. Recovery is observation-only through command 7
        for the same sequence ID.
        """
        seq = self.next_sequence()

        def recover_status(timeout: float = 2.5) -> dict:
            deadline = time.monotonic() + timeout
            samples = []
            last_error = None
            while time.monotonic() < deadline:
                try:
                    ...
                    if r["status"] != STATUS_OK:
                        return {
                            ...
                            "state": s.get("state"),
                            "samples": samples,
                            "reason": f"bridge status {r['status']}",
                        }
                    state = s.get("state")
                    if state in (INPUT_STATE_ACCEPTED, INPUT_STATE_IN_PROGRESS):
                        return {
                            "active": True,
                            "state": state,
                            "samples": samples,
                    ...
                    if state in (
                        INPUT_STATE_COMPLETED,
                        INPUT_STATE_ALREADY_COMPLETED,
                        INPUT_STATE_ABORTED,
                        INPUT_STATE_NOT_FOUND,
                    ):
                        return {
                            "active": False,
                            "state": state,
                            "samples": samples,
                            "reason": "latch is not active",
                        }
            ...
            return {
                ...
                "samples": samples,
                "timeout": True,
                "reason": last_error or "status recovery timeout",
            }
        ...
        try:
            ...
            expected = (
                f"UDP timeout command={CMD_INPUT_HID_LATCH} "
                f"request_id={seq}"
            )
            if str(exc) != expected:
                raise
        ...
        if r["status"] != STATUS_OK:
            raise BridgeError(
                f"HID latch rejected status={r['status']} result={r['result']}"
            )

        state = initial.get("state")
        active = state in (INPUT_STATE_ACCEPTED, INPUT_STATE_IN_PROGRESS)
        if not active:
            status = recover_status()
            active = bool(status.get("active"))
        else:
            status = {"active": True, "state": state, "samples": []}
